"""JUCEMAT — Junta Comercial — stub.

Esta Junta não expõe a relação de leiloeiros em HTML estruturado / sem
captcha. Stub registrado para que o registry conheça a sigla e o runner
relate o status como `requires_manual_request`.
"""

from __future__ import annotations

from .base import StubJuntaScraper


class Jucemat(StubJuntaScraper):
    sigla = "JUCEMAT"
    uf = "MT"
    base_url = "https://www.jucemat.mt.gov.br"
    reason = (
        "requires_manual_request: JUCEMAT publica apenas portarias e calendário de prova; lista de leiloeiros não exposta."
    )
